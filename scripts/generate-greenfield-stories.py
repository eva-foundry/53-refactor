...
Generates 500+ Greenfield build stories from architecture decisions.
Stories are NOT migrations - they are NEW implementations using patterns.
"""
...
def generate_stories():
    """Generate comprehensive Greenfield WBS with 500+ stories."""
    ...
    stories = {
        ...
        "summary": {
            "total_epics": 11,
            "total_stories": 0,  # Will be calculated
            "total_fp": 0,  # Will be calculated
        }
    }
    ...
    # Size to FP mapping
    size_to_fp = {"S": 5, "M": 8, "L": 13, "XL": 21}
    
    for epic in stories["epics"]:
        total_stories += len(epic["stories"])
        for story in epic["stories"]:
            total_fp_value += size_to_fp.get(story["size"], 8)
    
    stories["summary"]["total_stories"] = total_stories
    stories["summary"]["total_fp"] = total_fp_value
    
    return stories

if __name__ == "__main__":
    stories_data = generate_stories()
    
    print(f"[OK] Generated {stories_data['summary']['total_stories']} Greenfield build stories")
    print(f"[OK] Total story points: {stories_data['summary']['total_fp']} FP")
    print(f"[OK] Organized into {stories_data['summary']['total_epics']} epics")
    print("\nEpic Breakdown:")
    for epic in stories_data["epics"]:
        epic_stories = len(epic["stories"])
        epic_fp = sum([5 if s["size"] == "S" else 8 if s["size"] == "M" else 13 if s["size"] == "L" else 21 for s in epic["stories"]])
        print(f"  {epic['epic_id']}: {epic['epic_name']} ({epic_stories} stories, {epic_fp} FP)")
